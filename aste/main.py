...
class SpanModelTrainer(BaseModel):
    root: Path
    train_kwargs: dict
    path_config: Path = Path(os.path.join(common_path.project_dir, "training_config/aste.jsonnet")).resolve()
    ...
    def setup_data(self):
        for data_split in [SplitEnum.train, SplitEnum.dev, SplitEnum.test]:
            ...
            data.load(data_split == SplitEnum.train)
...
def main(
    root=os.path.join(common_path.project_dir, 'data/triplet_data'),
    names=("14lap",),
    seeds=(0,),
    sep=",",
    name_out="results",
