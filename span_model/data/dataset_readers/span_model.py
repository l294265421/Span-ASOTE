...
class SpanModelReader(DatasetReader):
    ...
    def _process_tags(self, sent) -> List[str]:
        ...
        result = self.tag_maker.run(spans, labels, num_tokens=len(sent.text))
        return result
    ...
    def _process_sentence(self, sent: Sentence, dataset: str):
        ...
        if sent.ner is not None:
            ...
            labels_temp = self._process_tags(sent)
            if len(labels_temp) == 0:
                labels_temp = ['O'] * len(text_field.tokens)
            fields["tag_labels"] = SequenceLabelField(
                labels_temp, text_field, label_namespace=f"{dataset}__tag_labels"
            )
